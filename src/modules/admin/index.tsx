...
const AdminPanel = () => {
    ...
    const deleteUserHistory = async () => {
        const response = await deleteUserHistoryApi('68a53c652949fda42bc67efa');
        console.log("Fetched Users:", response);
    }
    useEffect(() => {
        getAllUsers();
                    ...
                    <tbody>
                        {users?.map((user, idx) => {
                            return (
                                ...
                                >
                                    <td className="py-3 font-semibold text-purple-900 text-center">{idx + 1}</td>
                                    <td className="py-3 font-semibold text-purple-900 text-center">{user?.firstName}</td>
                                    <td className="py-3 font-semibold text-purple-900 text-center">{user?.lastName}</td>
            ...
            {showModal && selectedUser && (
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 overflow-y-auto">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 md:p-10 w-full max-w-3xl relative border-2 border-purple-200 overflow-y-auto h-screen">
                        ...
                        <h2 className="text-3xl font-bold mb-2 text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-500">{`${selectedUser?.firstName} ${selectedUser?.lastName} `}</h2>
